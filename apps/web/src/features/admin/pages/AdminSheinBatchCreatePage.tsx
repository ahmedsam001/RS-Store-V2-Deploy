...
import { ArrowLeft, CheckCircle2, CircleDollarSign, ClipboardList, ExternalLink, PackageCheck, RefreshCw, Search, Truck } from 'lucide-react';
...
export function AdminSheinBatchCreatePage() {
  ...
  const availableShowingFrom = availableMeta && availableMeta.total > 0 ? (availableMeta.page - 1) * availableMeta.limit + 1 : 0;
  const availableShowingTo = availableMeta ? Math.min(availableMeta.page * availableMeta.limit, availableMeta.total) : 0;
  ...
  const totals = useMemo(() => {
    ...
    const totalSar = selectedReadyItems.reduce((sum, item) => sum + getUnitSarAmount(getSelectedUnitSarInput(item, selectedItemSarAmounts)) * item.quantity, 0);
  ...
  const missingPrices = selectedReadyItems.filter((item) => getUnitSarAmount(getSelectedUnitSarInput(item, selectedItemSarAmounts)) <= 0);
  ...
  function toggleAvailableItem(item: AdminAvailableSheinOrderItem) {
    setSelectedItemIds((current) => {
      ...
      if (suggestedUnitSar) {
        setSelectedItemSarAmounts((amounts) => (amounts[item.id] ? amounts : { ...amounts, [item.id]: suggestedUnitSar }));
      }
  ...
  function goToStep(next: WizardStep) {
    ...
    if (next > 2 && !canGoStep3) {
      setNotice({ type: 'error', message: 'Review or add a valid Unit SAR for every selected item and a valid exchange rate' });
      return;
    }
    setStep(next);
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid gap-3 md:grid-cols-4">
        ...
        <AdminMetricCard title="Total SAR" value={totalsNeedPrices ? 'Pending' : formatMajorMoney(totals.totalSar, 'SAR')} icon={CircleDollarSign} hint={totalsNeedPrices ? 'Enter SAR prices in Step 2' : 'SHEIN cost'} tone={totalsNeedPrices ? 'gold' : 'success'} />
        <AdminMetricCard title="Total EGP" value={totalsNeedPrices ? 'Pending' : formatMajorMoney(totals.totalEgp, 'EGP')} icon={CircleDollarSign} hint={totalsNeedPrices ? 'Enter SAR prices in Step 2' : 'SAR × exchange rate'} tone={totalsNeedPrices ? 'gold' : 'success'} />
      </section>
          ...
          {step === 1 ? (
            <AdminCard title="Step 1 Select ready items" description="Only orders with approved deposit appear here" contentClassName="space-y-4">
              <form
                ...
                onSubmit={(event) => {
                  event.preventDefault();
                  loadAvailableItems(availableSearch, 1).catch((error) => setNotice(toNotice(error)));
                }}
              >
                ...
                <Button type="button" variant="ghost" onClick={() => loadAvailableItems(availableSearch, availablePage).catch((error) => setNotice(toNotice(error)))}><RefreshCw className="h-4 w-4" aria-hidden="true" />Refresh</Button>
              </form>

              {availableMeta ? (
                <div className="flex flex-wrap items-center justify-between gap-2 text-xs font-bold text-muted-foreground">
                  <span>
                    Showing {availableShowingFrom}-{availableShowingTo} of {availableMeta.total} ready item(s)
                  </span>
              ...
              <div className="grid gap-2">
                {readyItems.map((item) => {
                  ...
                  return (
                    <article key={item.id} className="grid gap-3 rounded-2xl border border-[#efd6c5] bg-white p-3 transition hover:bg-[#fffaf3] md:grid-cols-[auto_minmax(0,1fr)_auto] md:items-start">
                      <input
                        type="checkbox"
              ...
              {availableMeta && availableMeta.totalPages > 1 ? (
                <AdminPagination meta={availableMeta} onPageChange={(page) => loadAvailableItems(availableSearch, page).catch((error) => setNotice(toNotice(error)))} />
              ) : null}

              <div className="flex justify-end">
                <Button type="button" disabled={!canGoStep2} onClick={() => goToStep(2)}>Next Review SAR Prices</Button>
              </div>
            </AdminCard>
          ) : null}

          {step === 2 ? (
            <AdminCard title="Step 2 Review SAR prices" description="Unit SAR is filled from the saved SHEIN import price, and you can edit it when SHEIN changes the price" contentClassName="space-y-4">
              ...
              <div className="grid gap-2">
                {selectedReadyItems.map((item) => {
                  const unitSar = getUnitSarAmount(getSelectedUnitSarInput(item, selectedItemSarAmounts));
                  ...
                  return (
                    <article key={item.id} className="grid gap-3 rounded-2xl border border-[#efd6c5] bg-white p-3 md:grid-cols-[minmax(0,1fr)_180px_180px] md:items-center">
                      <div className="min-w-0">
                        ...
                        <ItemVariantMeta item={item} />
                        <div className="mt-2"><SheinProductButton item={item} /></div>
                      </div>
              ...
              {missingPrices.length > 0 ? (
                <AdminSoftPanel className="text-sm font-bold text-[#7a4a12]">
                  {missingPrices.length} selected item(s) still need a valid Unit SAR amount because no saved SHEIN import price was found.
                </AdminSoftPanel>
              ) : null}
...
function SuggestedUnitSarNote({ item }: { item: AdminAvailableSheinOrderItem }) {
  ...
  if (!suggested) {
    return <p className="mt-1 text-[11px] font-bold text-amber-700">No saved SHEIN import price for this item</p>;
  }
...
function SheinProductButton({ item }: { item: AdminAvailableSheinOrderItem }) {
  ...
  if (!url) {
    return (
      <Button type="button" variant="outline" size="sm" disabled title="No SHEIN product link saved for this product">
        Missing SHEIN Link
      </Button>
    );
  }
...
function ItemVariantMeta({ item }: { item: AdminAvailableSheinOrderItem }) {
  ...
  return (
    <div className="mt-2 flex flex-wrap gap-1.5 text-[11px] font-black text-[#5f4638]">
      {details.map((detail) => (
        <span key={`${detail.label}-${detail.value}`} className="rounded-full border border-[#efd6c5] bg-[#fffaf3] px-2 py-1">
...
function getVariantDetails(item: AdminAvailableSheinOrderItem) {
  const variantName = firstText(item.productVariantNameSnapshot, item.productVariant?.nameEn, item.productVariant?.nameAr);
  ...
  const sku = firstText(item.productVariantSkuSnapshot, item.productVariant?.sku, item.productSkuSnapshot);
...
function getSelectedUnitSarInput(item: AdminAvailableSheinOrderItem, amounts: Record<string, string>) {
...
function getSuggestedUnitSarInput(item: AdminAvailableSheinOrderItem) {
  const numeric = Number(String(item.suggestedUnitSarAmount ?? '').replace(/,/g, '').trim());
