...
export function AdminCustomOrdersPage() {
  ...
  async function review(id: string, event: FormEvent<HTMLFormElement>, status: 'ACCEPTED' | 'REJECTED') {
  ...
  return (
    <div className="space-y-6">
      ...
      <AdminCard title="Queue Filters" description="Use status links and search to find requests quickly">
            ...
            {STATUS_FILTERS.map((filter) => (
              <Link
                ...
                className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 text-sm font-black transition ${
                  status === filter.value ? 'border-[#d9a441] bg-[#fff7df] text-[#8a5a10]' : 'border-border bg-card text-muted-foreground hover:bg-accent'
                }`}
              >
                {filter.label}
                <AdminCountBadge count={filter.value === 'ALL' ? total : counts[filter.value] ?? 0} />
              </Link>
            ))}
          </div>
      ...
      {!loading && !error ? (
        <AdminCard title="Requests" description={`${total} matching custom order request${total === 1 ? '' : 's'}`}>
...
function CustomOrderAdminCard({
  ...
  reviewing: boolean;
  onReview: (id: string, event: FormEvent<HTMLFormElement>, status: 'ACCEPTED' | 'REJECTED') => void;
}) {
  return (
      ...
      <div className="flex flex-col gap-4 sm:flex-row">
        <div className="w-full shrink-0 sm:w-36">
          {item.customerImageUrl ? (
            <img src={item.customerImageUrl} alt="" className="aspect-square w-full rounded-xl object-cover" />
          ) : (
        ...
        <div className="min-w-0 flex-1 space-y-3">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-muted-foreground">{item.user?.phone ?? item.user?.email ?? 'No contact saved'}</p>
          </div>
          <a className="inline-flex max-w-full items-center gap-2 break-all text-sm font-bold text-rs-gold" href={item.productUrl} target="_blank" rel="noreferrer">
          ...
          {item.customerNote ? <p className="rounded-xl bg-muted p-3 text-sm leading-6 text-muted-foreground">{item.customerNote}</p> : null}
        </div>
      </div>
      ...
      >
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="grid gap-1 text-xs font-black text-rs-ink">
            Admin title
            <input className="rounded-xl border border-input bg-card px-3 py-2 text-sm" name="adminTitle" defaultValue={item.adminTitle ?? ''} />
          </label>
          <label className="grid gap-1 text-xs font-black text-rs-ink">
            Admin image
            <input className="rounded-xl border border-input bg-card px-3 py-2 text-sm" name="adminImage" type="file" accept="image/*" />
          </label>
          <label className="grid gap-1 text-xs font-black text-rs-ink">
            Product price
            <input className="rounded-xl border border-input bg-card px-3 py-2 text-sm" name="adminPriceAmount" defaultValue={minorToInput(item.adminPriceAmount)} placeholder="0.00" />
          </label>
          <label className="grid gap-1 text-xs font-black text-rs-ink">
            Shipping
            <input className="rounded-xl border border-input bg-card px-3 py-2 text-sm" name="adminShippingAmount" defaultValue={minorToInput(item.adminShippingAmount)} placeholder="0.00" />
          </label>
          <label className="grid gap-1 text-xs font-black text-rs-ink sm:col-span-2">
            Final total
            <input className="rounded-xl border border-input bg-card px-3 py-2 text-sm" name="adminTotalAmount" defaultValue={minorToInput(item.adminTotalAmount)} placeholder="0.00" />
          </label>
        </div>
        <label className="grid gap-1 text-xs font-black text-rs-ink">
          Admin note
          <textarea className="min-h-24 rounded-xl border border-input bg-card px-3 py-2 text-sm" name="adminNote" defaultValue={item.adminNote ?? ''} placeholder="Internal note or rejection reason" />
        </label>
        {item.adminImageUrl ? (
          <a className="text-xs font-bold text-rs-gold" href={item.adminImageUrl} target="_blank" rel="noreferrer">Current admin image</a>
        ) : null}
        {item.convertedOrder ? (
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
          ...
          <Button type="submit" name="reviewStatus" value="ACCEPTED" disabled={reviewing || Boolean(item.convertedOrderId)}>
